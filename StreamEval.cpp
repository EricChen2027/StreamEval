...
void Format2Text(unsigned int errNum, vector<NALU_t> vNalu, STATIC_INFO statInfo, MEDIA_INFO mediaInfo){
    ...
    printf("|                    Flv Tag Info                            |\n");
    printf("|++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++|\n");
    printf("|TagNum    |TagType   |TagSize    |TimeStamp   |TimeStampSub |\n");
    ...
    if (lVInvalid > 0) {
        ...
        for (vector<TAG_INFO>::iterator it = statInfo.vInvalid.begin(); it != statInfo.vInvalid.end(); ++it) {
            printf("    Invalid Video Tag Size             : %d\n", (*it).tagsize);
	        }
    }
    ...
    if (lAVInvalid > 0) {
        ...
        for (vector<TAG_INFO>::iterator it = statInfo.vInvalid.begin(); it != statInfo.vInvalid.end(); ++it) {
            printf("    Invalid Audio Tag Size             : %d\n", (*it).tagsize);
        }
    }
    //nuber of unsync
    ...
    if (video_only > 0) {
		printf("\n");
        printf("- There are %d(Total:%d) frames that only video tag could play!\n", video_only, statInfo.videoTagNum);
    }
	uint32_t audio_only = statInfo.audioOnly;
	if (audio_only > 0) {
		printf("\n");
		printf("- There are %d(Total:%d) tags that only audio could play!\n", audio_only, statInfo.audioTagNum);
	}
